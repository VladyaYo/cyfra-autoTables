...
def pivot_client_data():
   ...
   ##
    ...
    df_merged = df1.merge(
        ga_df[["Ідентифікатор трансакції", "Джерело й тип трафіку сеансу"]],  # Берём только нужные колонки из df2
    ...
    df_merged.rename(columns={"Джерело й тип трафіку сеансу": "источник"}, inplace=True)
